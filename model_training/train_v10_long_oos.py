"""
V10 Long Out-of-Sample Test — 5yr Train, 12mo Test

Tests V10 Cross-Scale Convergence features against V8/V9 baseline.
Uses V7 d8 winner config (depth=8, lr=0.02, l2=15, cw=[0.5,2,2]).
Multi-seed (42, 123, 777) for robustness.

Split:
  Train: 2020-02-01 -> 2025-01-01 (5 years)
  Val:   2025-01-01 -> 2025-02-01 (1 month, for early stopping + threshold selection)
  Embargo: 2025-02-01 -> 2025-02-08 (7 days)
  Test:  2025-02-08 -> 2026-02-15 (12 months, truly unseen)
...
Usage:
  python model_training/train_v10_long_oos.py
"""
import sys
import os
...
RESULTS_DIR = Path(__file__).parent / "results_v10" / "long_oos"
...
# Thresholds to evaluate (V10: added @0.75 and @0.80)
...
def save_plots(y_proba, y_test, test_times, threshold_results, seed, output_dir,
    ...
    ax.set_title(f'V10 Long OOS Confusion — s{seed}')
    fig.tight_layout()
    ...
    ax.set_title(f'V10 Long OOS ROC — s{seed} (Macro AUC={macro_auc:.3f})')
    ...
    ax.set_title(f'V10 Long OOS PR — s{seed}')
    ...
    ax.set_title(f'V10 Long OOS Equity — s{seed} (all thresholds)')
    ...
    if val_selected_threshold is not None:
        ...
        if best_key in threshold_results:
            ...
            if len(eq) > 1:
                ...
                ax.set_xlabel('Trade #')
                ax.set_ylabel('Cumulative Equity (%)')
                ax.set_title(f'V10 Long OOS Equity — s{seed} @{val_selected_threshold:.2f} '
                             f'(profit={profit:.1f}%)')
...
def _save_monthly_breakdown(trades_detail, test_times, seed, output_dir, threshold):
    ...
    ax.set_title(f'V10 Long OOS Monthly Profit — s{seed} @{threshold:.2f}')
...
def save_cross_seed_equity(seed_results, output_dir):
    ...
    ax.set_xlabel('Trade #')
    ax.set_ylabel('Cumulative Equity (%)')
    ax.set_title('V10 Long OOS — Val-Selected Threshold')
    ax.legend(loc='upper left')
    ax.grid(True, alpha=0.3)
    ax.axhline(y=0, color='black', linewidth=0.5, linestyle='--')

    # Right: fixed @0.70 (production threshold)
    ax = axes[1]
    for sr in seed_results:
        seed = sr['seed']
    ...
    ax.set_xlabel('Trade #')
    ax.set_ylabel('Cumulative Equity (%)')
    ax.set_title('V10 Long OOS — Fixed @0.70')
    ax.legend(loc='upper left')
    ax.grid(True, alpha=0.3)
    ax.axhline(y=0, color='black', linewidth=0.5, linestyle='--')
...
def run_long_oos():
    ...
    logger.info("=" * 90)
    logger.info("V10 LONG OUT-OF-SAMPLE TEST — Cross-Scale Convergence")
    logger.info(f"  Config: V7 d8 (depth={MODEL_PARAMS['depth']}, iter={MODEL_PARAMS['iterations']})")
    logger.info(f"  Seeds: {SEEDS}")
    logger.info(f"  Train: {SPLIT['train_start'].date()} -> {SPLIT['train_end'].date()} (5yr)")
    ...
    logger.info(f"  Test:  {SPLIT['embargo_end'].date()} -> {SPLIT['test_end'].date()} (12mo)")
    ...
    logger.info(f"    F3 corr dynamics: {n_corr_vel + 1}")  # +1 for xtf_corr_agreement
    ...
    for seed in SEEDS:
        ...
        # @0.70 metrics (production threshold)
        ...
        logger.info(f"\n  => s{seed}: AUC={threshold_results[f'{THRESHOLDS[0]:.2f}']['roc_auc_macro']:.3f} | "
                    ...
                    f"@0.70: {at070.get('total_profit_pct', 0):.1f}% ({at070.get('n_trades_simulated', 0)} trades) | "
        ...
        # Save trade logs (at val-selected AND @0.70)
        save_trade_log(y_proba_test, y_test, test_times, val_best_thresh,
                       seed, RESULTS_DIR, cooldown=TRADE_COOLDOWN)
        if abs(val_best_thresh - 0.70) > 0.001:
            # Also save @0.70 trade log
            y_pred_070 = np.argmax(y_proba_test, axis=1)
            for i in range(len(y_pred_070)):
                if y_pred_070[i] in TRADE_CLASSES:
                    if y_proba_test[i, y_pred_070[i]] < 0.70:
                        y_pred_070[i] = 0
            rows_070 = []
            cumulative = 0.0
            next_allowed = 0
            for i in range(len(y_pred_070)):
                if y_pred_070[i] == 0 or i < next_allowed:
                    continue
                gain = TP_PCT if y_pred_070[i] == y_test[i] else -SL_PCT
                cumulative += gain
                rows_070.append({
                    'timestamp': str(test_times.iloc[i]),
                    'predicted': CLASS_NAMES[int(y_pred_070[i])],
                    'actual': CLASS_NAMES[int(y_test[i])],
                    'confidence': float(y_proba_test[i, y_pred_070[i]]),
                    'gain_pct': gain,
                    'cumulative_equity': cumulative,
                })
                next_allowed = i + TRADE_COOLDOWN
            if rows_070:
                pd.DataFrame(rows_070).to_csv(
                    RESULTS_DIR / f"s{seed}_trades_at070.csv", index=False)
    ...
    results_path = RESULTS_DIR / "v10_long_oos_results.json"
    ...
    # ------------------------------------------------------------------
    logger.info(f"\n{'='*90}")
    logger.info(f"V10 LONG OOS COMPLETE — {elapsed:.0f}s ({elapsed/60:.1f}min)")
    logger.info(f"  AUC: {mean_auc:.3f} +/- {std_auc:.3f}")
    logger.info(f"  Honest (val-selected): {mean_honest:.1f}% mean ({honest_profits})")
    logger.info(f"  @0.70 (production):    {mean_070:.1f}% mean ({profits_070})")
